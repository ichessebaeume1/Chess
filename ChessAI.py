...
def find_best_move(gamestate, valid_moves):
    ...
    next_move = None
    find_move_nega_max_alpha_beta(gamestate, valid_moves, DEPTH, -CHECKMATE, CHECKMATE, 1 if gamestate.white_to_move else -1)
    return next_move
...
# positive score == white is good, negative score == black is winning
def score_board(gamestate):
    ...
    for row in range(len(gamestate.board)):
        for col in range(len(gamestate.board[row])):
            ...
            if square[0] == "w":
                score += piece_value[square[1]] + piece_position_score * .1
            elif square[0] == "b":
                score -= piece_value[square[1]] + piece_position_score * .1
